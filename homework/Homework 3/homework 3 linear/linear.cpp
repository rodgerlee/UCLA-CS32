...
//  homework 3 no 2
//
//  Created by Rodger Lee on 2/6/20.
...
bool anyFalse(const double a[], int n)
{
    ...
    //recursion
    return anyFalse(a+1, n - 1);  // This is not always correct.
...
int countTrue(const double a[], int n)
{
    ...
    // true = 0, false = 1;
    int t = somePredicate(a[0]);
    
    return countTrue(a + 1, n - 1) + t;
}
...
int positionOfSmallest(const double a[], int n)
{
    ...
    int smallest = positionOfSmallest(a + 1, n - 1);

    if  (min(a[0], a[smallest + 1]) == a[0])
        return 0;
