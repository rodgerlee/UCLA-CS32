...
string Game::takePlayerTurn()
{
    for (;;)
    {
        cout << "Your move (n/e/s/w/x/h or nothing): "; //TODO: add h. COMPLETED
        string playerMove;
        ...
        else if (playerMove.size() == 1)
        {
            if (tolower(playerMove[0]) == 'x')
                ...
                return player->move(dir);
            else if (tolower(playerMove[0]) == 'h') // applied condition for history
            {
                ...
                if (cin.get() == '\n')
                    clearScreen();
                m_arena->display("");
            }//TODO: add history display implentation
